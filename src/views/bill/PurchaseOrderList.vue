      ...
      handleExport() {
        ...
        downFile('/depotHead/export', param).then((data)=> {
          ...
          if (typeof window.navigator.msSaveBlob !== 'undefined') {
            window.navigator.msSaveBlob(new Blob([data],{type: 'application/vnd.ms-excel'}), '采购订单列表导出.xls')
          }else{
            let url = window.URL.createObjectURL(new Blob([data],{type: 'application/vnd.ms-excel'}))
            let link = document.createElement('a')
            link.style.display = 'none'
            link.href = url
            link.setAttribute('download', '采购订单列表导出_' + new Date().getTime() + '.xls')
      ...
      handleDetailModal(record) {
        ...
        findBillDetailByNumber({ number: record.number }).then((res) => {
          if (res && res.code === 200) {
            ...
            this.$refs.modalForm.billSubType = '采购订单'
            this.$refs.modalForm.prefixNo = 'CGDD'
            this.$refs.modalForm.disableSubmit = true; // 禁用保存按钮
            this.$refs.modalForm.rowCanEdit = false;  // 禁用编辑
            this.$refs.modalForm.title = "采购订单-查看";
