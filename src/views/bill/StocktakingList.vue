    ...
    <a-col :md="24">
      <a-card :style="cardStyle" :bordered="false">
        ...
        <div class="table-operator"  style="margin-top: 5px">
          ...
          <a-tooltip placement="left" title="可以进行库存盘点。" slot="action">
  ...
  export default {
    ...
    methods: {
      ...
      handleExport() {
        ...
        let searchObj = {}
        // 添加单据类型筛选到查询参数中
        let exportQueryParam = {...this.queryParam, type: "盘点", subType: "盘点"}
        ...
        downFile('/depotHead/export', param).then((data)=> {
          ...
          if (typeof window.navigator.msSaveBlob !== 'undefined') {
            window.navigator.msSaveBlob(new Blob([data],{type: 'application/vnd.ms-excel'}), '盘点单列表导出.xls')
          }else{
            let url = window.URL.createObjectURL(new Blob([data],{type: 'application/vnd.ms-excel'}))
            let link = document.createElement('a')
            link.style.display = 'none'
            link.href = url
            link.setAttribute('download', '盘点单列表导出_' + new Date().getTime() + '.xls')
