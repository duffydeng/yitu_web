  ...
  export default {
    ...
    methods: {
      ...
      handleExport() {
        ...
        let searchObj = {}
        // 添加单据类型筛选到查询参数中
        let exportQueryParam = {...this.queryParam, type: "其它", subType: "拆卸单"}
        ...
        downFile('/depotHead/export', param).then((data)=> {
          ...
          if (typeof window.navigator.msSaveBlob !== 'undefined') {
            window.navigator.msSaveBlob(new Blob([data],{type: 'application/vnd.ms-excel'}), '拆卸单列表导出.xls')
          }else{
            let url = window.URL.createObjectURL(new Blob([data],{type: 'application/vnd.ms-excel'}))
            let link = document.createElement('a')
            link.style.display = 'none'
            link.href = url
            link.setAttribute('download', '拆卸单列表导出_' + new Date().getTime() + '.xls')
